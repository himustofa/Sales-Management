...
public class SaleDAO implements ISaleDAO {
	...
	public int deleteEntities(Map<String, Object> request) {
		...
		try {
			...
			if (uuids.size() == 0) {
				query = sessionFactory.getCurrentSession().createQuery("DELETE FROM Sale");
			} else {
				query = sessionFactory.getCurrentSession().createQuery("DELETE FROM Sale WHERE id NOT IN(:suppIds)");
	...
	@Override
	@Transactional
	public Sale getEntityById(UUID id) {
		Sale entity = (Sale) sessionFactory.getCurrentSession().get(Sale.class, id);
		return entity;
	}

	@Override
	@Transactional
	public UUID updateEntity(Sale entity) {
	...
	public List<Sale> getList(Map<String, String> map) {
		...
		Query query = sessionFactory.getCurrentSession().createQuery("FROM Sale");
